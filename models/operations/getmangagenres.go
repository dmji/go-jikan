package operations

import (
	"github.com/dmji/go-jikan/models/components"
)

type GetMangaGenresRequest struct {
	// Filter genres by type
	Filter *components.GenreQueryFilter `queryParam:"style=form,explode=true,name=filter"`
}

func (o *GetMangaGenresRequest) GetFilter() *components.GenreQueryFilter {
	if o == nil {
		return nil
	}
	return o.Filter
}

type GetMangaGenresResponse struct {
	HTTPMeta components.HTTPMetadata `json:"-"`
	// Returns entry genres, explicit_genres, themes and demographics
	Genres *components.Genres
}

func (o *GetMangaGenresResponse) GetHTTPMeta() components.HTTPMetadata {
	if o == nil {
		return components.HTTPMetadata{}
	}
	return o.HTTPMeta
}

func (o *GetMangaGenresResponse) GetGenres() *components.Genres {
	if o == nil {
		return nil
	}
	return o.Genres
}
